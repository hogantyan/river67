...
$(document).ready(function() {
	...
	$('#count').click(function() {
		...
		let count = [];
		let participationCount = []; // кол-во участий NEW
		const valueTable = {
		...
		for (const i in herbGathering) {
			...
			for (let locs in now.catsLocs) {
				...
				for (const cat of nowCats) {
					...
					}	// Количество участий NEW
                	const participationInstance = _.find(participationCount, {id: cat.id});
	                if (!participationInstance) {
	                 	participationCount.push({id: cat.id, participations: 1});
	}               else {
		participationInstance.participations++;
	}
        ...
        participationCount.sort((a, b) => b.participations - a.participations);
		// Ищем активистов
        ...
        for (const i in participationCount) {
          	const cat = participationCount[i];
        	val += `${cat.id}\t${cat.participations}\n`;
         }
